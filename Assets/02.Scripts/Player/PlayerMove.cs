...
public class PlayerMove : MonoBehaviour // 클래스 이름 PlayermOVE를 PlayerMove로 수정하여 파일 이름과 일치시킵니다.
{
    ...
    void Update()
    {
        ...
        float h = Input.GetAxisRaw("Horizontal"); // 수평 입력에 대한 값을 -1, 0, 1로 가져온다. (A/D 또는 왼쪽/오른쪽 화살표)
        float v = Input.GetAxisRaw("Vertical");   // 수직 입력에 대한 값을 -1, 0, 1로 가져온다. (W/S 또는 위/아래 화살표)
